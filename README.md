# Scheduler
Scheduler service that ensures the buffer streams are sent through an specific dataflow
...
Also, there's a python script at `./scheduler/send_msgs_test.py` to do some simple manual testing, by sending msgs to the service stream key.
...
But in order to make the automatic docker image build work, you'll need to set the `SIT_PYPI_USER` and `SIT_PYPI_PASS` variables in the Gitlab CI setting page: [Scheduler CI Setting Page](https://gitlab.insight-centre.org/sit/mps/scheduler/settings/ci_cd).
